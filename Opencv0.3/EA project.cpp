...
/* Resetiraj algoritam na klik miša */
...
{
	VideoCapture cam;
	Mat frame, image, maskBackproj;
	RotatedRect faceRRect;
	...
	/*if (!SP->IsConnected())
	{
		cout << "We're not connected to USB port!" << endl;
		return 1;
	}*/
	...
	while (true){
		...
		updateHistogram(frame, trackWindow, Scalar(MIN(hmin, hmax), MIN(smin, smax), MIN(vmin, vmax)), 
											Scalar(MAX(hmin, hmax), MAX(smin, smax), MAX(vmin, vmax)));
		double hCenter = frame.rows / 2;
		double wCenter = frame.cols / 2;
		...
		while (trackObject && !startSetup)
		{
			...
			try
			{
				...
				faceRRect = CamShift(backproj, trackWindow, TermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 100, 0.01));
				rectangle(frame, trackWindow, Scalar(255, 255, 0));
				
				#pragma region Serial comunication

				/* Seriska komunikacija*/
				if (faceRRect.center.x > (wCenter + 25)) { SP->WriteData("c", 1);}
				else if (faceRRect.center.x < (wCenter - 25)) { SP->WriteData("x", 1); }
				if (faceRRect.center.y >(hCenter + 25)) { SP->WriteData("y", 1); }
				else if (faceRRect.center.y < (hCenter - 25)) { SP->WriteData("a", 1); }
